-- Phase 2: add the system-only "Assumed Resolved" ticket status.
--
-- An admin-engaged ticket that has been quiet for 7 days is moved to
-- "Assumed Resolved" by a periodic sweep (see assumed-resolved.ts +
-- assumeResolveQuietTickets in server.ts). It counts as a resolution in the
-- rate but is kept SEPARATE from a human "Resolved" (new assumedResolvedCount).
-- A new user message reopens it to "In Review".
--
-- Two changes, both additive/reversible:
--   1. Widen tickets_status_check to accept 'Assumed Resolved'.
--   2. CREATE OR REPLACE tickets_stats (carrying the migration-016 body
--      forward) to add 'assumedResolvedCount'. activeCount already excludes the
--      new status because it is not in the active-status list — no change there.
...
SELECT jsonb_build_object(
  ...
  -- Spam/Irrelevant). 'Assumed Resolved' is NOT active (not listed here).
  ...
  -- Median response time (one 18-day outlier was dragging the mean to 8.4 h
  -- while the true median sits at ~6.5 min).
  ...
  -- System auto-resolutions, kept separate from human 'Resolved'. The JS layer
  -- adds this to the resolution-rate numerator.
