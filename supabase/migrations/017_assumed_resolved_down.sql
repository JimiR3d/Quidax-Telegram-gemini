-- Revert Phase 2 "Assumed Resolved".
--
-- NOTE (dev-only): the constraint re-add will FAIL if any ticket still has
-- status = 'Assumed Resolved'. This down migration first parks those rows back
-- to 'In Review' (and clears the assumed-close resolved_at) so the narrower
-- constraint can be restored. Run only against a non-production / recovery DB.

UPDATE public.tickets
SET status = 'In Review', resolved_at = NULL, updated_at = now()
WHERE status = 'Assumed Resolved';
...
-- Restore the migration-016 tickets_stats body (no assumedResolvedCount).
