-- Phase 1 KPI corrections (no schema change, no ingestion change):
--   1a. avgResponseMs → medianResponseMs using percentile_cont(0.5) so one
--       outlier (18-day ticket) can no longer drag the headline figure.
--   1b. activeCount now excludes General Question / Praise / Spam/Irrelevant
--       tickets from the resolution-rate denominator — these are noise, not
--       support work. Moves the headline rate from ~16% to ~22% honestly.
--   1c. Remove the dead 'Classifying' branch from activeCount; the DB CHECK
--       constraint has never allowed that status value.
...
SELECT jsonb_build_object(
  ...
  -- Spam/Irrelevant) so banter and price-commands don't inflate the queue or
  -- drag the resolution rate. The dead 'Classifying' status is also gone.
  ...
  -- while the true median sits at ~6.5 min (p90 = 67 min).
