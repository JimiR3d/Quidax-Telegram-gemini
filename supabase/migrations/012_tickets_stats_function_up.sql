-- Milestone 4: move KPI aggregation into the database.
-- The old /api/tickets implementation pulled up to 5,000 ticket rows into
-- Node and counted them in JS — past 5,000 tickets every KPI would silently
-- undercount. This function computes the same numbers in SQL over the FULL
-- filtered set, plus the per-day volume series (Lagos calendar days) the
-- dashboard chart previously derived from the raw rows.
--
-- Filter parameters mirror applyBaseFilters + applyStatsFilters in server.ts.
-- "Today" boundaries are passed in by the server (computed in Africa/Lagos)
-- so day-boundary logic stays in one place.
-- NULL-handling matches the old JS exactly: IS DISTINCT FROM for the
-- "not Resolved/Dismissed" urgency cards (JS used !==), and empty-string
-- categories fold into 'Uncategorized' (JS used `category || "Uncategorized"`).
...
SELECT jsonb_build_object(
  ...
  'activeCount',        count(*) FILTER (WHERE status IN ('Open', 'In Review', 'Escalated', 'Awaiting User', 'Classifying')),
  ...
  'avgResponseMs',      (SELECT round(avg(extract(epoch FROM (first_admin_reply_at - created_at)) * 1000))
                           FROM filtered
