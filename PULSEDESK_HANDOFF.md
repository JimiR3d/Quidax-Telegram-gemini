...
*   **Milestone 5 (2026-06-12): Automated Status Update Bot (PRD Feature 4) — built, verified in dry-run, ships OFF.**
    ...
    *   **Rollout state: Phase A (local dry-run proof) DONE. Phase B pending:** set `BOT_REPLIES_ENABLED=true` on Railway (dry-run stays on by default) and review real "WOULD send" logs + `bot_replies` rows. **Phase C pending:** flip `BOT_REPLIES_DRY_RUN=false` together, then verify one controlled real reply (threaded correctly, no ADMIN_REPLY self-pollution). Kill switch any time: unset/false `BOT_REPLIES_ENABLED` + restart.
...
*   Env vars to set in the Railway dashboard (values from local `.env`): `GROQ_API_KEY`, `GEMINI_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `DASHBOARD_PASSWORD`, `TELEGRAM_API_ID`, `TELEGRAM_API_HASH`, `TELEGRAM_SESSION_STRING`, `TELEGRAM_GROUP_USERNAME`, `TELEGRAM_ADMIN_USER_IDS`, `NODE_ENV=production`, `DEMO_MODE=false`; optional: `JIRA_BASE_URL`, `JIRA_EMAIL`, `JIRA_API_TOKEN`, `JIRA_PROJECT_KEY`, `HEARTBEAT_URL`, `SUPPORT_API_KEY`; **outbound bot (Milestone 5, leave unset until rollout):** `BOT_REPLIES_ENABLED` (kill switch, default off) and `BOT_REPLIES_DRY_RUN` (default true — flip to `false` only after reviewing production dry-run logs; this makes the bot post to the real group).
