...
if (missingVars.includes("DASHBOARD_PASSWORD") && process.env.VITE_DASHBOARD_PASSWORD) {
...
var logger = {
  info: (component, msg, data) => console.log(
    JSON.stringify({
      level: "info",
      ts: /* @__PURE__ */ (/* @__PURE__ */ new Date()).toISOString(),
      component,
      msg,
      ...data && { data }
    })
  ),
  warn: (component, msg, data) => console.warn(
    JSON.stringify({
      level: "warn",
      ts: /* @__PURE__ */ (/* @__PURE__ */ new Date()).toISOString(),
      component,
  ...
  error: (component, msg, data) => console.error(
    JSON.stringify({
      level: "error",
      ts: /* @__PURE__ */ (/* @__PURE__ */ new Date()).toISOString(),
      component,
...
var CircuitBreaker = class {
  constructor(name, failureThreshold = 3, successThreshold = 1, openDurationMs = 3e4) {
    this.name = name;
...
function shouldProcessMessage(text, learnedKeywords = /* @__PURE__ */ new Set()) {
...
async function refreshLearnedKeywords(supabase) {
  try {
    const { data, error } = await supabase.from("learned_keywords").select("keyword").gte("frequency", 2).eq("active", true).order("frequency", { ascending: false }).limit(200);
    if (error) return;
...
async function extractAndLearnKeywords(supabase, text) {
  try {
    ...
    for (const keyword of unique) {
      const { data: existing } = await supabase.from("learned_keywords").select("frequency").eq("keyword", keyword).maybeSingle();
      if (existing) {
        await supabase.from("learned_keywords").update({ frequency: existing.frequency + 1, updated_at: now }).eq("keyword", keyword);
      } else {
        await supabase.from("learned_keywords").insert({ keyword, frequency: 1, active: true, updated_at: now });
      }
    }
...
var DEMO_TICKETS = [
  ...
  },
  {
    ...
    suggested_reply: "Thanks for providing the transaction ID. I can see the on-chain confirmation \u2014 I'm checking with our internal team why this hasn't been credited yet and will have an update for you within 30 minutes.",
  ...
  },
  {
    ...
    is_complaint: true,
    status: "Open",
    raw_text: "I just swapped BTC to USDT and was charged a 1.5% fee. I thought the fee was 0.5%. When did this change? Nobody told us.",
    suggested_action: "Explain current fee structure. Check if fee was applied correctly. Provide changelog link.",
    suggested_reply: "Hi! Our fee structure was updated last month \u2014 you can see the full breakdown at quidax.com/fees. The 1.5% applied to your swap is correct under the new tier. We apologise if the communication wasn't clear enough.",
  ...
  },
  {
    ...
    is_complaint: false,
    status: "Resolved",
    raw_text: "Just want to say the new QDX staking feature is amazing! Got my first rewards this morning. Keep it up team! \u{1F680}",
    suggested_action: "Acknowledge and thank user. No further action required.",
    suggested_reply: "Thank you so much, this means a lot to the team! Really glad you're enjoying staking. More features coming soon \u{1F64C}",
...
async function startServer() {
  ...
  function getAuthContext(req) {
    ...
    if (legacyKey) {
      ...
      if (process.env.SUPPORT_API_KEY && timingSafeStringCompare(legacyKey, process.env.SUPPORT_API_KEY)) {
        return {
          role: "support",
  ...
  async function logAuditAction(supabase, actorId, action, target, oldState, newState, ip) {
    try {
  ...
  const TELEGRAM_ADMIN_USER_IDS = (process.env.TELEGRAM_ADMIN_USER_IDS || "").split(",").map((s) => s.trim()).filter(Boolean);
  let cachedAdminsByGroup = /* @__PURE__ */ new Map();
  let lastAdminFetchByGroup = /* @__PURE__ */ new Map();
  async function checkIsAdmin(groupId, senderId) {
    if (!senderId) return false;
    const sId = String(senderId);
    if (TELEGRAM_ADMIN_USER_IDS.includes(sId)) return true;
    const cachedAdmins2 = cachedAdminsByGroup.get(groupId);
    const lastAdminFetch2 = lastAdminFetchByGroup.get(groupId) || 0;
    if (cachedAdmins2 && cachedAdmins2.has(sId)) return true;
    if (!tlClient) return false;
    if (Date.now() - lastAdminFetch2 > 1e3 * 60 * 15) {
      try {
    ...
    const finalAdmins = cachedAdminsByGroup.get(groupId) || /* @__PURE__ */ new Set();
    return finalAdmins.has(sId);
  }
  ;
  async function generateSuggestedReply(text, classification) {
    if (!genAI) return "";
    return geminiBreaker.call(
      () => withTimeout(
        (async () => {
...
Classify the user message below. Do NOT default to General Question unless the user is genuinely only asking for information.`;
  ...
  async function processAndIngestMessage(text, telegramId, groupId, replyToMsgId, msgDate, isAdminSender, telegramDeepLink, skipPreFilter, senderId) {
    ...
    const msgDateISO = msgDate ? new Date(msgDate * 1e3).toISOString() : /* @__PURE__ */ (/* @__PURE__ */ new Date()).toISOString();
    const isResolution = /\b(thanks|thank you|resolved|fixed|worked|solved|appreciate)\b/i.test(
      text
    );
    if (isResolution && !isAdminSender) {
      try {
        const { data: recentTickets } = await supabase.from("tickets").select("*").eq("sender_hash", senderHash).in("status", ["Open", "In Review"]).order("created_at", { ascending: false }).limit(1);
...
${text}
[/USER_REPLY]`;
          await supabase.from("tickets").update({ raw_text: newRawText, status: "Resolved" }).eq("id", parentTicket.id);
          logger.info(
            "Ingestion",
...
[/ADMIN_REPLY]`;
        ...
        try {
          const { data: parentMsg } = await supabase.from("messages").select("id, raw_text").eq("telegram_message_id", String(replyToMsgId)).maybeSingle();
          let parentTicket = null;
          if (parentMsg) {
            const { data: pt } = await supabase.from("tickets").select("*").eq("message_id", parentMsg.id).maybeSingle();
            parentTicket = pt;
            if (!parentTicket && parentMsg.raw_text) {
              const safeText = parentMsg.raw_text.replace(/[%_]/g, "");
              const { data: potentialTickets } = await supabase.from("tickets").select("*").ilike("raw_text", `%${safeText}%`).limit(1);
...
${text}
[/USER_REPLY]`;
            await supabase.from("tickets").update({ raw_text: newRawText }).eq("id", parentTicket.id);
            logger.info(
              "Ingestion",
    ...
    if (telegramId && !String(telegramId).startsWith("rand_")) {
      const { data: existingMsg } = await supabase.from("messages").select("id").eq("telegram_message_id", String(telegramId)).maybeSingle();
      if (existingMsg) {
        logger.debug(
          "Ingestion",
    ...
    const { data: dbMessage, error: msgError } = await supabase.from("messages").insert({
      ...
      raw_text: text,
      message_timestamp: msgDateISO,
      ingested_at: /* @__PURE__ */ (/* @__PURE__ */ new Date()).toISOString(),
      sender_hash: senderHash
    }).select("id").single();
    if (msgError) {
    ...
    if (telegramId && groupId) {
      ...
      ticketInsert.telegram_deep_link = telegramDeepLink || buildTelegramDeepLink(groupId, telegramId);
    }
    const { data: dbTicket, error: ticketError } = await supabase.from("tickets").insert(ticketInsert).select("*").single();
    if (ticketError) {
    ...
    (async () => {
      try {
        ...
        const finalStatus = isResolution ? "Resolved" : isPreFiltered || isAutoDismiss ? "Dismissed" : needsEscalation ? "In Review" : "Open";
      ...
      try {
        ...
        client.addEventHandler(async (event) => {
          ...
          try {
            ...
            const inTarget = chat && (chat.username === targetGroup || chat.title?.includes(targetGroup) || chat.title?.toLowerCase().includes("quidax"));
            if (inTarget) {
        ...
        client.addEventHandler(async (update) => {
          if (update.className === "UpdateEditMessage" || update.className === "UpdateEditChannelMessage") {
            ...
            try {
              const supabase = getSupabase();
              const { data: msgRow } = await supabase.from("messages").update({
                raw_text: msg.message,
                edited_at: /* @__PURE__ */ (/* @__PURE__ */ new Date()).toISOString()
              }).eq("telegram_message_id", String(msg.id)).select("id").single();
              if (msgRow) {
                await supabase.from("tickets").update({ raw_text: msg.message }).eq("message_id", msgRow.id);
              }
              logger.info(
                "Telegram",
          ...
          if (update.className === "UpdateDeleteMessages" || update.className === "UpdateDeleteChannelMessages") {
            ...
            try {
              ...
              for (const msgId of deletedIds) {
                const { data: msg } = await supabase.from("messages").select("id").eq("telegram_message_id", String(msgId)).maybeSingle();
                if (msg) {
                  await supabase.from("tickets").update({ status: "Dismissed" }).eq("message_id", msg.id);
                  logger.info(
                    "Telegram",
...
`
  ...
  app.get("/api/tickets", requireAuth, async (req, res) => {
    if (process.env.DEMO_MODE === "true") {
      ...
      if (req.query.issues_only === "true") {
        demo = demo.filter(
          (t) => !NON_ESSENTIAL_CATEGORIES.has(t.category) && t.urgency !== "Low"
        );
      }
    ...
    try {
      ...
      let query = supabase.from("tickets").select("*", { count: "exact" }).order("created_at", { ascending: false }).range(from, to);
      let statsQuery = supabase.from("tickets").select("id, status, urgency, created_at, category").order("created_at", { ascending: false }).limit(5e3);
      const applyBaseFilters = (q) => {
        ...
        if (req.query.issues_only === "true") {
          ...
          temp = temp.or(
            `summary.eq."Processing message...",category.not.in.(${nonEssStr})`
          );
        }
      ...
      const [{ data, error, count }, { data: statsData, error: statsError }] = await Promise.all([query, statsQuery]);
      if (error) throw error;
      ...
      const stats = {
        ...
        resolvedTodayCount: resolved.filter(
          (t) => isToday(new Date(t.updated_at || t.created_at))
        ).length,
        resolvedCount: resolved.length,
        criticalCount: allData.filter(
          (t) => t.urgency === "Critical" && t.status !== "Resolved" && t.status !== "Dismissed"
        ).length,
        highCount: allData.filter(
          (t) => t.urgency === "High" && t.status !== "Resolved" && t.status !== "Dismissed"
        ).length,
        mediumCount: allData.filter(
          (t) => t.urgency === "Medium" && t.status !== "Resolved" && t.status !== "Dismissed"
        ).length,
        lowCount: allData.filter(
          (t) => t.urgency === "Low" && t.status !== "Resolved" && t.status !== "Dismissed"
        ).length,
        ticketsTodayCount: allData.filter(
          (t) => isToday(new Date(t.created_at))
        ).length,
        totalCount: count ?? 0,
    ...
    try {
      ...
      const jiraPayload = {
        fields: {
          ...
          priority: {
            name: ticket.urgency === "Critical" ? "Highest" : ticket.urgency === "High" ? "High" : "Medium"
          },
          labels: [
            "pulsedesk",
      ...
      if (!jiraRes.ok) {
        ...
        try {
          jiraErrDetail = JSON.parse(errText)?.errorMessages?.[0] || jiraErrDetail;
        } catch {
        }
  ...
  app.post("/api/backfill", heavyLimiter, requireAuth, async (req, res) => {
    try {
      ...
      const cutoffDate = days ? Math.floor(Date.now() / 1e3) - days * 24 * 60 * 60 : 0;
          ...
          while (retries < 3) {
            try {
              ...
              if (msg_text.includes("429") || msg_text.toLowerCase().includes("rate limit")) {
                ...
                const waitMs = retryAfterMatch ? parseInt(retryAfterMatch[1]) * 1e3 : delay * 2;
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const tId = telegramId ? Number(telegramId) : Math.floor(Math.random() * 1e7);
