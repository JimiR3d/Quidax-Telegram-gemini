...
*   **What Works now (2026-06-16):** all critical/high audit findings from 2026-06-12 are resolved. Live Telegram ingestion runs on two parallel paths: `getChannelDifference` (15s poll, ~14s lag, primary — `CHANNEL_DIFF_ENABLED=true` on Railway) and AutoFetch (3-min sweep, ~37–174s lag, safety net). Session #3 active after two prior rotations; rolling-deploy session-overlap is prevented by SIGTERM graceful-disconnect + 60s startup delay. Conversation grouping (subtasks 1–6 of 7) folds a user's follow-up messages into one ticket. All known accuracy gaps closed (Pidgin, auto-dismiss policy, orphaned-thread retro-ingest, admin-message drop, admin auto-resolve). Mobile layout fixed. Dashboard no longer rate-limits itself. **Still weak:** deep nuanced sub-categories for Quidax edge-cases; long-term GramJS session persistence (expiry interval unknown); unquoted admin replies arriving >90s after the ticket (or when multiple tickets land in the same window) still can't be matched reliably; the /train Skip button (subtask 7) is pending.
