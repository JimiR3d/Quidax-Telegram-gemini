...
*   **What Works now (2026-07-03, DEPLOYED commit `c786fd1`) — Community Chat taxonomy + fairer urgency:** the classifier now separates benign community chatter (greetings, banter, price talk — new "Community Chat" category, auto-dismissed and excluded from every KPI) from genuine scams/ads (which is all "Spam/Irrelevant" means now). Urgency grading was reworked: a platform-wide outage counts as High even when asked as a question, context-free fragments can never grade above Medium, and a vague "money lost" claim without a described transaction is High rather than Critical. A brand-new Critical ticket lands as Open (with its ESCALATED flag) instead of falsely showing as "Admin Replied". The accuracy benchmark was re-baselined once for the new taxonomy: 24 cases, and both the local and production runs scored 100% on category, urgency, and overall (old 20-case scores are not comparable). Migration 022 also stopped one legacy admin-created ticket from inflating the resolved count.
*   **What Works now (2026-06-22, DEPLOYED commit `d7be948`) — rate-honesty Phase 3 "Handed off":** admin replies that redirect a user off-platform ("send an email to support@quidax.com" / "DM me") now move the ticket to a dedicated `Handed off` status, excluded from BOTH the active denominator and the resolution numerator — the rate no longer counts work PulseDesk structurally cannot observe closing, nor penalises itself for it. Detection is a tested pure module (`handoff-detect.ts`), set in `reclassifyFromAdminReply` (guarded to Open/In Review), reopens to In Review on a new in-channel user reply, and the dashboard shows a "Handed Off" chip/filter. Migration `020` widened the status constraint and added `handedOffCount`. Backfilled 5 existing email hand-offs (preview-first); resolution rate 45.6%→46.3%. All docs updated for pitch: `KPI_DEFINITIONS.md`, `ARCHITECTURE.md`, `DEMO_SCRIPT.md`, `INTERVIEW_PREP.md`, `README.md`, Master Guide + .docx.
