...
*   **Milestone 5 (Outbound):** the Automated Status Update Bot (Feature 4) — dashboard status changes post an empathetic reply into the Telegram thread via GramJS, behind a kill switch, dry-run mode, per-status templates, rate limiting, dedup, and age/admin/group guards. — **BUILT & VERIFIED IN DRY-RUN (2026-06-12)**; production rollout (enable dry-run on Railway → review logs → flip live together) still pending.
