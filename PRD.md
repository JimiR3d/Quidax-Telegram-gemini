...
*   **Milestone 6 (Post-Audit Hardening):** fix the critical/high findings from the 2026-06-12 full system audit (KNOWN_ISSUES §6), one commit per item. Done so far: silent message drops (Fix 1), cross-chat edit/delete corruption (Fix 2), AutoFetch ordering (Fix 3), auto-dismiss policy (Fix 4), quoted-parent retro-ingest (Fix 5), live-listener resurrection + watchdog (Fix 6), and the **AI benchmark now ships to production** (Fix 7, `c8b2ea9`) — restoring the measurement tool behind the "AI Accuracy" success metric (§8). Queued: Fix 8 (Gemini suggested-reply failures). — **IN PROGRESS (2026-06-13)**
