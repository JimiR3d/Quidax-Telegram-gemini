...
*   **Milestone 5 (Outbound, proposed):** the Automated Status Update Bot (Feature 4) — dashboard status changes post an empathetic reply into the Telegram thread via GramJS. Needs explicit safety rails (dry-run mode, per-status templates, rate limiting, and a kill switch) since this is the first feature that WRITES to the real community group.
